...
class ReflectionXHPChildrenDeclaration {
	...
	public function getExpression(): ReflectionXHPChildrenExpression {
		...
		throw new Exception("Tried to get child expression for XHP class ".:xhp::class2element(\get_class($this->context)).", but it does not have an expressions.");
	}
...
class ReflectionXHPChildrenExpression {
	public function __construct(private string $context, private array<int, mixed> $data) {
	}
	...
	<<__Memoize>>
	public function getSubExpressions(): (ReflectionXHPChildrenExpression, ReflectionXHPChildrenExpression) {
		$type = $this->gettype();
		invariant(
			$type === XHPChildrenExpressionType::SUB_EXPR_SEQUENCE || $type === XHPChildrenExpressionType::SUB_EXPR_DISJUNCTION,
		...
		try {
			...
			return tuple(new ReflectionXHPChildrenExpression($this->context, $sub_expr_1), new ReflectionXHPChildrenExpression($this->context, $sub_expr_2));
		} catch (IncorrectTypeException $_) {
			// handled below
		}
	...
	public function getConstraintString(): string {
		...
		invariant(
			$type === XHPChildrenConstraintType::ELEMENT || $type === XHPChildrenConstraintType::CATEGORY,
	...
	public function getSubExpression(): ReflectionXHPChildrenExpression {
		invariant($this->getConstraintType() === XHPChildrenConstraintType::SUB_EXPR, 'Only expression constraints have a single sub-expression - in %s', $this->context);
		$data = $this->data[2];
		try {
			$data = TypeSpec\dict_like_array(TypeSpec\int(), TypeSpec\mixed())
				->assertType($this->data[2]);
		...
		throw new Exception('Expected a sub-expression, got a '.(\is_object($data) ? \get_class($data) : \gettype($data)).' - in '.$this->context);
	}
