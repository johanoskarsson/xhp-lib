...
abstract final class XHPAttributeCoercion {
	private static XHPAttributeCoercionMode $mode = XHPAttributeCoercionMode::THROW_EXCEPTION;
	...
	private static function LogCoercion(\x\:composable-element $context, string $what, string $attr, mixed $val): void {
		switch (self::GetMode()) {
			...
			case XHPAttributeCoercionMode::LOG_DEPRECATION:
				...
				\trigger_error(
					\sprintf('Coercing value of type `%s` to `%s` for attribute `%s` of '.'element `%s`', $val_type, $what, $attr, :xhp::class2element(\get_class($context))),
					\E_USER_DEPRECATED,
				);
				return;
	...
	public static function CoerceToString(\x\:composable-element $context, string $attr, mixed $val): string {
	...
	public static function CoerceToInt(\x\:composable-element $context, string $attr, mixed $val): int {
		self::LogCoercion($context, 'int', $attr, $val);
		if ((($val is string) && \is_numeric($val) && $val !== '') || ($val is float)) {
			return (int)$val;
		}
	...
	public static function CoerceToBool(\x\:composable-element $context, string $attr, mixed $val): bool {
	...
	public static function CoerceToFloat(\x\:composable-element $context, string $attr, mixed $val): float {
