...
class ReflectionXHPAttribute {
	...
	public function getValueClass(): string {
		...
		$v = $this->extraType;
		invariant($v is string, 'Class name for attribute %s is not a string', $this->getName());
		return $v;
	}
	...
	public function getEnumValues(): Set<string> {
		...
		$v = $this->extraType;
		invariant(\is_array($v), 'Class name for attribute %s is not a string', $this->getName());
		return new Set($v);
	}
	...
	public static function IsSpecial(string $attr): bool {
		return \strlen($attr) >= 6 && $attr[4] === '-' && self::$specialAttributes->contains(\substr($attr, 0, 4));
	}
